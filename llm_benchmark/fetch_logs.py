...
async def fetch_logs(run_id: str):
    all_logs = []
    ...
    initial_page = await client.ai_gateway.logs.list(
        ...
        filters=json.dumps(
            [
                Filter(key="metadata.value", operator="eq", value=[run_id]),
                Filter(key="success", operator="eq", value=[True]),
            ]
        ),
    ...
    total_pages = (initial_page.result_info.total_count // 50) + 1
    print(f"Total results: {initial_page.result_info.total_count}")
